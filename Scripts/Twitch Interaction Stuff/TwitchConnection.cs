...
public partial class TwitchConnection : Node
{
    ...
    public async void Authenticate()
    {
        ...
        if (!LoadConfig())
        {
            GD.Print("Loading Config failed!");
        ...
        if (Godot.FileAccess.FileExists(path))
        {
            ...
            if (token.ContainsKey("scope") && scopes.Length != 0)
            {
                ...
                {
                    GD.Print("The token's scope length does not align with the amount of required scopes.");
                    GetToken();
                ...
                else
                {
                    foreach (var scope in scopes)
                    {
                        ...
                        {
                            GD.Print($"Scope {scope} was not included in the token's scope");
                            GetToken();
                            await ToSignal(this, SignalName.UserTokenReceived);
                            break;
                        }
            ...
            {
                GD.Print("Token does not contain \"scope\" field.");
                GetToken();
                await ToSignal(this, SignalName.UserTokenReceived);
            }
        }
        ...
        {
            GD.Print("User token file does not exist.");
            GetToken();
            await ToSignal(this, SignalName.UserTokenReceived);
        }
        ...
        if (token.TryGetValue("access_token", out var access_token))
        {
            ...
            {
                GD.Print("Token invalid");
                if (token.TryGetValue("refresh_token", out var refresh_token))
                {
                    GD.Print($"refresh token: {refresh_token}");
                    if (GD.VarToStr(refresh_token) == string.Empty) GetToken();
                    else RefreshAccessToken(GD.VarToStr(refresh_token));
                ...
                {
                    GD.Print("No refresh token in dictionary");
                    EmitSignal(SignalName.AuthenticationFailed);
                    return;
                }
            }
        ...
        {
            GD.Print("No access token in dictionary");
            EmitSignal(SignalName.AuthenticationFailed);
        }
    }
    ...
    {
        GD.Print("Refreshing access token.");
        var request = new HttpRequest();
        AddChild(request);
        ...
        {
            GD.Print("Refreshing token failed, requesting new token");
            GetToken();
        }
        else
        {
            ...
            file.Close();
            EmitSignal(SignalName.UserTokenReceived, response);
        }
    }
    ...
    private bool LoadConfig()
    {
        ...
        if (error != Error.Ok)
        {
            GD.PrintErr("Loading auth config failed");
            return false;
        }
    ...
    private void ConnectWebSocket()
    {
        ...
        if (error == Error.Ok)
        {
            GD.Print("Eventsub connected to websocket!");
    ...
    public async void SubscribeToEvent(string eventName, int version, Dictionary conditions)
    {
        ...
        var request = new HttpRequest();
        AddChild(request);
        GD.Print($"Subscribing to event {eventName}");
        ...
        request.QueueFree();
        var response = (Dictionary)Json.ParseString(reply[3].AsByteArray().GetStringFromUtf8());
        GD.Print($"Code: {(int)reply[1]} Response: {response}");
        if (response.ContainsKey("error"))
        {
            GD.Print($"Subscribing to event {eventName} failed: Error {response["status"]} ({response["error"]}): {response["message"]}");
            return;
        }
        GD.Print($"Now listening to {eventName}");
    }
            ...
            case WebSocketPeer.State.Closed:
                {
                    if (eventsubConnected)
                    {
                        ...
                        GD.Print($"Websocket closed with code {code}: {reason}. Clean: {code != -1}");
