...
public partial class TwitchConnection : Node
{
	...
	private async void IsTokenValid(string token)
	{
		...
		{
			GD.Print("[IsTokenValid] Validation failed.");
	...
	private async void GetBadgesFromURL(string url)
	{
        ...
        GD.Print("[GetChatBadges] Sending Request");
		...
		//var file = Godot.FileAccess.Open("user://test.txt", Godot.FileAccess.ModeFlags.Write);
		//string jsonText = ((string)Json.ParseString(answer[3].AsByteArray().GetStringFromUtf8())).Replace('<', '"').Replace('>', '"');
		//file.StoreString(jsonText);

		if ((int)answer[1] == 200)
		{
			var badgesData = (Dictionary)Json.ParseString(((string)Json.ParseString(answer[3].AsByteArray().GetStringFromUtf8())).Replace('<', '"').Replace('>', '"'));
